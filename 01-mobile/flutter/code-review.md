...
3. **Architecture/consistency** — does it respect the layer/slice boundaries in [architecture-principles](architecture-principles.md) and the conventions in [flutter.md](flutter.md)?
...
- Enums persisted or transmitted by `.index` instead of `.name` ([coding-standards](coding-standards.md)).
  ```dart
...
- Confirm `flutter analyze` and the test suite pass locally (see [git-conventions](../../00-global/git-conventions.md)).
