...
Universal rules that apply to every Flutter project. [flutter.md](flutter.md) only adds on top of this — it never contradicts it.
...
- Default to the **UI / Domain / Data** architecture (MVVM) described in [architecture-principles](architecture-principles.md) for every project — the layers and folder layout are not a per-project choice.
- **Never create a folder that ends up empty.** The `ui/`, `domain/`, `data/` tree shown in [flutter.md](flutter.md) is the *shape* a project converges toward, not a scaffold to stamp out up front — create a folder (including a feature's `domain/use_cases/` at all) only at the moment it actually gets its first file.
...
- **Static analysis** — `flutter analyze` (or `dart analyze`) passes clean (no new errors or warnings introduced by the change) — see [flutter.md](flutter.md)'s `## Static analysis` section. Never skip this assuming "it looks fine" or because the change was small.
- **Tests** — pass locally; if the change touches logic covered by [flutter.md](flutter.md)'s `## Testing` criteria, new tests were written for it.
...
- When tests are warranted, use the testing stack specified in [flutter.md](flutter.md)'s `## Testing` section.
