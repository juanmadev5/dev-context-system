...
- Never design or implement a screen/Composable assuming one fixed device size. The realistic range of phone/tablet/foldable sizes is in scope **by default** — unlike i18n (see [jetpack-compose.md](jetpack-compose.md)'s "No hardcoded strings or dimensions" section), this isn't a per-project question to ask, it's a standing requirement for any UI work.
