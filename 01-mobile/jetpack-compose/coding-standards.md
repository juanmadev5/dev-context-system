...
Universal rules that apply to every Jetpack Compose project. [jetpack-compose.md](jetpack-compose.md) only adds on top of this — it never contradicts it.
...
- Default to the **UI / Domain / Data** architecture (MVVM) described in [architecture-principles](architecture-principles.md) for every project — the layers are not a per-project choice.
- **Never create a folder that ends up empty.** The `core/`, `feature/<feature>/{domain/, data/, presentation/}` tree shown in [jetpack-compose.md](jetpack-compose.md) is the *shape* a project converges toward, not a scaffold to stamp out up front — create a folder (including a feature's `domain/` at all) only at the moment it actually gets its first file.
...
- **Static analysis** — `./gradlew lint` and `./gradlew detekt` pass clean (no new errors or warnings introduced by the change) — see [jetpack-compose.md](jetpack-compose.md)'s `## Static analysis` section. Never skip this assuming "it looks fine" or because the change was small.
- **Tests** — pass locally; if the change touches logic covered by [jetpack-compose.md](jetpack-compose.md)'s `## Testing` criteria, new tests were written for it.
...
- When tests are warranted, use the testing stack specified in [jetpack-compose.md](jetpack-compose.md)'s `## Testing` section.
