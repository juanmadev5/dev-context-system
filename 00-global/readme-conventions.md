...
2. **Stack** — brief bullets (frontend/backend/DB/infra). Just enough to orient without opening `CLAUDE.md` or the repository — don't duplicate the detail that already lives there.
3. **Prerequisites** — concrete versions needed to run the project (Node/pnpm, .NET SDK, Docker, Postgres, etc.).
4. **Getting started** — steps to bring the whole thing up end to end. For a multi-service project (backend + frontend, or more), this is `docker compose up` from the root — see [docker](../04-infra/docker.md)'s root `docker-compose.yml` convention. If a given service needs its own specific setup beyond that (e.g. a backend service's own config-with-secrets template — see that project's stack note for the exact file), link to that service's own README instead of inlining the detail here.
