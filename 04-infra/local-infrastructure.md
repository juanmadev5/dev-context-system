...
- **From the host** (an app run directly, not in Docker): `localhost` + the published port for each service.
...
- `start-dev` Keycloak mode is dev-only; never used as a production template.
