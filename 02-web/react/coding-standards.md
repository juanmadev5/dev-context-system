...
Universal rules that apply to every React project. [react.md](react.md) only adds on top of this — it never contradicts it.
...
- **Never create a folder that ends up empty.** The `shared/`, `features/<feature>/` tree shown in [react.md](react.md) is the *shape* a project converges toward, not a scaffold to stamp out up front — create a folder only at the moment it actually gets its first file.
...
- **Static analysis** — passes clean (no new errors or warnings introduced by the change) — see [react.md](react.md)'s `## Static analysis` section. Never skip this assuming "it looks fine" or because the change was small.
- **Tests** — pass locally; if the change touches logic covered by [react.md](react.md)'s `## Testing` criteria, new tests were written for it.
...
- When tests are warranted, use the testing stack specified in [react.md](react.md)'s `## Testing` section.
