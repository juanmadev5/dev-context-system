---
tags: [react, coding-standards]
---

# Coding Standards — React

Universal rules that apply to every React project. [[02-web/react/react|react.md]] only adds on top of this — it never contradicts it.
...
- Never inline literal strings or numbers that carry meaning (route paths, storage keys, config values, thresholds, etc.).
- Always extract them to named constants, or to a TypeScript `enum`/union of string literals when the value represents a closed set of options.
- When persisting or transmitting an enum (local storage, JSON, query params), always use its **name**, never a numeric value. Numeric values silently break when a member is added, removed, or reordered; names are stable and self-documenting across boundaries.

## DRY

- Duplication is a defect, not a style preference. If the same logic (not just similar-looking code) appears more than once, extract it — a function, a shared component, a custom hook, a utility module, whatever fits.
- DRY applies to logic and business rules, not to superficial structural similarity. Do not force an abstraction over code that merely looks alike but represents different concerns — that creates false coupling. See [[02-web/react/architecture-principles|architecture-principles]] for how this interacts with premature abstraction.

## SOLID

Applied together with DRY, not instead of it — see [[02-web/react/architecture-principles|architecture-principles]] for how these interact with layer/slice boundaries.

- **Single Responsibility**: a component/hook/function has one reason to change. If describing what something does requires "and", it's a candidate to split.
- **Open/Closed**: extend behavior by adding new code (a new component variant, a new case), not by modifying working code to special-case a new scenario — especially across module boundaries.
...
- **Interface Segregation**: don't force a consumer to depend on props/methods it doesn't use. Prefer several small, focused interfaces over one large one.
- **Dependency Inversion**: high-level/business logic depends on abstractions, not on concrete infrastructure. Same rule as [[02-web/react/architecture-principles|architecture-principles]]'s "dependency direction" — SOLID and the architecture layering reinforce each other, they're not separate concerns.

Apply these pragmatically: they're a guide for keeping code changeable, not a checklist to satisfy for its own sake. This doesn't apply to injected dependencies — those always get an interface, from the first implementation, per [[02-web/react/architecture-principles|architecture-principles]]'s dependency rule. It's about internal structure: don't split a component into multiple pieces or add extra indirection with no real boundary or reason to change — that's premature abstraction, which [[02-web/react/architecture-principles|architecture-principles]] already warns against.

## Architecture

- Default to **Clean Architecture** or **Vertical Slice** depending on project size/complexity. See [[02-web/react/architecture-principles|architecture-principles]] for the decision criteria and layout.
- **Never create a folder that ends up empty.** The `shared/`, `features/<feature>/` tree shown in [[02-web/react/react|react.md]] is the *shape* a project converges toward, not a scaffold to stamp out up front — create a folder only at the moment it actually gets its first file.
...
- **Exception to "never comment on what"**: add a short JSDoc comment (`/** */`) above a function when it's genuinely complex or abstract enough that a competent reader can't infer its purpose/approach from the name + signature + a read of the body alone. This is not an invitation to comment everything:
  ...
  - Multi-step chains of higher-order functions (e.g. a `.map().filter().reduce()`-style pipeline with several transformations stacked).
  - Generic/abstract code (complex generics, higher-order components, render-prop or Strategy-style patterns where intent isn't clear without seeing how it's used).
...
- **Static analysis** — passes clean (no new errors or warnings introduced by the change) — see [[02-web/react/react|react.md]]'s `## Static analysis` section. Never skip this assuming "it looks fine" or because the change was small.
- **Tests** — pass locally; if the change touches logic covered by [[02-web/react/react|react.md]]'s `## Testing` criteria, new tests were written for it.
- **Self-review** — the full diff was read start to finish before calling the task done, per [[02-web/react/code-review|code-review]]'s self-review section.
- **Docs** — the project's README was updated if the change affects it ([[00-global/readme-conventions|readme-conventions]]); `docs/SOURCES.md` was updated if a source not already covered by [[02-web/react/sources|sources]] was consulted.
...
- When tests are warranted, use the testing stack specified in [[02-web/react/react|react.md]]'s `## Testing` section.
...
- **Never guess an API, method, or parameter** that hasn't been verified against real code or official documentation ([[02-web/react/sources|sources]]) — that's always a case to check, never to fabricate.
...
- [[02-web/react/architecture-principles|architecture-principles]]
- [[02-web/react/code-review|code-review]]
- [[02-web/react/responsive-design|responsive-design]]
- [[02-web/react/sources|sources]]
