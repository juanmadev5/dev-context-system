...
3. **Architecture/consistency** — does it respect the layer/slice boundaries in [[02-web/react/architecture-principles|architecture-principles]] and the conventions in [[02-web/react/react|react.md]]?
4. **Performance** — unnecessary re-renders, missing memoization on expensive computations, unpaginated large payloads.
5. **Tests** — is coverage present where [[02-web/react/coding-standards|coding-standards]]'s testing criteria call for it?
6. **Style** — lowest priority, never blocking on its own.

## Severity classification

- **Blocking**: correctness bugs, security vulnerabilities, magic values (see [[02-web/react/coding-standards|coding-standards]]), duplicated logic, architecture-boundary violations, broken build/tests, missing error handling at a system boundary.
...
- Missing input validation at system boundaries (see [[02-web/react/coding-standards|coding-standards]]'s error-handling section).
...
- God components/functions doing more than one thing (violates Single Responsibility, see [[02-web/react/coding-standards|coding-standards]]).
- Domain/business objects leaking into the presentation layer, or any other [[02-web/react/architecture-principles|architecture-principles]] boundary violation.
  ```typescript
  ...
  ```
- Premature abstraction — extra internal splitting or indirection introduced with no real boundary or reason to change ([[02-web/react/architecture-principles|architecture-principles]]). This does **not** include interfaces on injected dependencies — those are mandatory from the first implementation; never flag a DI interface as premature just because there's only one concrete implementation today.
- Non-descriptive callback parameter names ([[02-web/react/coding-standards|coding-standards]]'s naming rules).
- Enums persisted or transmitted by numeric value instead of name ([[02-web/react/coding-standards|coding-standards]]).
  ```typescript
...
- Confirm type-checking, linting, and the test suite pass locally (see [[00-global/git-conventions|git-conventions]]).
