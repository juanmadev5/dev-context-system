...
Universal rules that apply to every Vue project. [vuejs.md](vuejs.md) only adds on top of this — it never contradicts it.
...
- **Never create a folder that ends up empty.** The `shared/` / `features/<feature>/components|composables|services|types` tree shown in [vuejs.md](vuejs.md) is the *shape* a project converges toward, not a scaffold to stamp out up front — create a folder only at the moment it actually gets its first file.
