...
3. **List that stack's folder** (e.g. `ls 01-mobile/flutter/`) and copy every file in it into the project's own `/docs/` folder. Every stack folder follows the same reading order: `architecture-principles.md`, `coding-standards.md`, `code-review.md`, `testing.md`, `error-handling.md`, `responsive-design.md` (frontend/mobile only), `sources.md`, then the stack's own note (e.g. `flutter.md`). Also copy:
   - [git-conventions](00-global/git-conventions.md) — every project needs it.
   - [dependency-management](00-global/dependency-management.md) — every project needs it.
   ...
   - whichever [04-infra/](04-infra/) notes the project actually pairs with (database, cache, auth, storage, containerization, security).
...
- `00-global/` — the notes that stay genuinely stack-agnostic: [git-conventions](00-global/git-conventions.md), [dependency-management](00-global/dependency-management.md), [readme-conventions](00-global/readme-conventions.md), [tech-stack-map](00-global/tech-stack-map.md) (the decision map, read once per project — not copied into a project's `/docs/`).
...
- `04-infra/` — one self-contained note per infrastructure piece: [docker](04-infra/docker.md), [postgresql](04-infra/postgresql.md), [redis](04-infra/redis.md), [keycloak-auth](04-infra/keycloak-auth.md), [aws-s3-storage](04-infra/aws-s3-storage.md), [local-infrastructure](04-infra/local-infrastructure.md), [security-practices](04-infra/security-practices.md).

Each mobile/web/backend stack folder bundles its own `architecture-principles.md`, `coding-standards.md`, `code-review.md`, `testing.md`, `error-handling.md`, `responsive-design.md` (frontend/mobile only), `sources.md`, and the stack note itself — see "Why the global rules are duplicated per stack" below.
